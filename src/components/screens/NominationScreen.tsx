...
export const NominationScreen: React.FC = () => {
  ...
  const [curatedMovies, setCuratedMovies] = useState<JustWatchMovie[]>([]);

  // Load curated movies on component mount
  useEffect(() => {
    const loadCuratedMovies = async () => {
      try {
        const response = await justWatchAPI.searchMovies(''); // Empty query returns curated list
        setCuratedMovies(response.items);
      } catch (error) {
        console.error('Error loading curated movies:', error);
      }
    };
  ...
  // Decide which movies to show: search results if searching, otherwise curated
  const moviesToShow = searchQuery.trim() ? searchResults : curatedMovies;

  const handleDirectNomination = async (movie: JustWatchMovie) => {
    ...
    if (selectedMovies.length >= config.max_nominations_per_user) {
      toast.error(`You can only select up to ${config.max_nominations_per_user} movies`);
      return;
    }
    ...
    try {
      ...
      const existingMovie = movies.find(m => m.justwatch_id === movieId);
      if (!existingMovie) {
        await addMovieToLibrary(movieId);
      }
      
      // Add to selection
      setSelectedMovies(prev => [...prev, movieId]);
      toast.success(`${movie.title} selected for nomination!`);
    } catch (error: any) {
      toast.error(error.message || 'Failed to select movie');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleMovieSelect = (movieId: string) => {
    setSelectedMovies(prev => {
      ...
      } else if (prev.length < config.max_nominations_per_user) {
        return [...prev, movieId];
      } else {
        toast.error(`You can only select up to ${config.max_nominations_per_user} movies`);
        return prev;
      ...
      <div className="p-4">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="text-white/80">Select up to {config.max_nominations_per_user} movies for tonight's vote</p>
            ...
            <>
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="text-white/70">
                  {selectedMovies.length}/{config.max_nominations_per_user} selected
                </div>
              </div>
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-6">
                  ...
                  {!searchQuery.trim() && (
                    <p className="text-white/60 text-sm">Click to nominate directly</p>
                  )}
                </div>
                ...
                {!searching && moviesToShow.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {moviesToShow.map((movie) => {
                      ...
                      const isInLibrary = movies.some(m => m.justwatch_id === movieId);
                      ...
                      return (
                        <motion.div
                          ...
                          whileTap={{ scale: 0.95 }}
                          onClick={() => handleDirectNomination(movie)}
                ...
                {!searching && searchQuery && moviesToShow.length === 0 && !searchError && (
                  <div className="text-center py-8">
